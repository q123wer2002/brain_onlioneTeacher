...
if($job=='buy')
{
	...
	if($step){
		if($postdb[price]<$rsdb[price]){
			showerr("妳出价不能低于系统规定的最低价:$rsdb[price]");
		}elseif($postdb[price]>$lfjdb[money]){
			showerr("妳出价不能大于妳自身的积分");
		}elseif($rsdb[wordnum]&&strlen($postdb[adword])>$rsdb[wordnum]){
			showerr("妳的广告文字内容字数不能大于{$rsdb[wordnum]}個");
		...
		add_user($lfjuid,-$postdb[price],'購買竞价广告扣分');	//扣除积分
		refreshto("?job=list","恭喜妳,广告提交成功",1);
	}
}
elseif($job=='list')
{
