...
if($job=='list')
{
	...
	while($rs = $db->fetch_array($query))
	{
		...
		if($_r=$db->get_one("SELECT * FROM {$pre}ad_norm_user WHERE u_endtime>'$timestamp' AND id='$rs[id]'"))
		{
			$_r[u_endtime]=date("Y-m-d H:i",$_r[u_endtime]);
			$rs[state]="{$_r[u_endtime]}过期";
			$rs[alert]="alert('直到{$_r[u_endtime]}以後才可購買');return false;";
			$rs[color]="#ccc;";
		}
		elseif($_r=$db->get_one("SELECT * FROM {$pre}ad_norm_user WHERE u_yz=0 AND id='$rs[id]'"))
		{
			$_r[u_endtime]=date("Y-m-d H:i",$_r[u_endtime]);
			$rs[state]="已有人購買,等待审核當中";
			$rs[alert]="alert('已有人購買,妳现在不能購買');return false;";
			$rs[color]="#ccc;";
		}
		elseif($rs[isclose]){
			$rs[state]="广告位已关闭";
...
elseif($job=='buy')
{
	$_r=$db->get_one("SELECT * FROM {$pre}ad_norm_user WHERE u_endtime>'$timestamp' AND id='$id'");
	
	if($_r)
	{
		$_r[u_endtime]=date("Y-m-d H:i",$_r[u_endtime]);
		showerr("直到{$_r[u_endtime]}才可購買");
	}
	...
	if($rsdb[autoyz]){
		$rsdb[_ifyz]='不需琯理员审核,直接显示';
	}else{
		$rsdb[_ifyz]='不能立即显示,需要等待琯理员审核';
	}
	require(ROOT_PATH."member/head.php");
...
elseif($action=="buy")
{
	...
	{
		showerr("妳的{$webdb[MoneyName]}不足$totalmoneycard,妳仅有{$webdb[MoneyName]}:$lfjdb[money]");
	}
	...
	if($u_yz)
	{
		...
		add_user($lfjuid,-$totalmoneycard,'購買普通广告位');
	}
	else
	{
	...
	$u_hits=0;
	$db->query("INSERT INTO `{$pre}ad_norm_user` ( `id` , `u_uid` , `u_username` , `u_day` , `u_begintime` , `u_endtime` , `u_hits` , `u_yz` , `u_code` , `u_money` , `u_moneycard` , `u_posttime` ) VALUES ('$id','$lfjuid','$lfjid','$atc_day','$u_begintime','$u_endtime','$u_hits','$u_yz','$u_code','$u_money','$u_moneycard','$timestamp')");
	make_ad_cache();
	refreshto("?job=list","購買成功,妳共支付了{$u_moneycard}{$webdb[MoneyName]}","3");
}
...
elseif($action=="edit")
{
	...
	if($rsdb[autoyz])
	{
		...
		if(!$rsdb[u_yz])
		{
			...
			{
				showerr("妳的{$webdb[MoneyName]}不足$totalmoneycard,妳仅有{$webdb[MoneyName]}:$lfjdb[moneycard]");
			}
		...
		else
		{
			...
			{
				showerr("妳的{$webdb[MoneyName]}不足,妳仅有{$webdb[MoneyName]}:$lfjdb[money]");
			}
			$money=abs($totalmoneycard-$rsdb[u_moneycard]);
			add_user($lfjuid,-$money,'購買普通广告位');
		}			
	}
	else
	{
