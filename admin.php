...
$typedb=array("color"=>"標題加亮","top"=>"置顶","untop"=>"取消置顶","del"=>"删除","undel"=>"從迴收站还原","fen"=>"评分","com"=>"推荐","uncom"=>"取消推荐","move"=>"转移栏目","movetop"=>"提前","movebottom"=>"置後","unyz"=>"取消验证","yz"=>"通过验证");
...
function do_work($job,$id){
	...
	{
		showerr("妳不能重复对本文进行{$typedb[$job]},文章標題是:$rsdb[title]");
	}
	...
	elseif($job=='move')
	{
		...
		{
			showerr("信息:“{$rsdb[title]}”,与目標栏目的模块不一样.不能转移栏目");
		}
	}
	elseif($job=='movetop')
	{
	...
	$array[title]=addslashes("妳的文章被{$typedb[$job]}了,文章標題是:$title");
	$url=get_info_url($rsdb[id],$rsdb[fid],$rsdb[city_id]);
	$array[content]=addslashes("操作理由是:$atc_reason<br><br>积分影响是:{$atc_fen}分<br><br><A HREF='$url' target=_blank>點擊查看</A>");
...
function list_allsort($fid,$Class){
	...
	while( $rs=$db->fetch_array($query) ){
		...
		if($web_admin||in_array($lfjid,explode(",",$rs[admin]))){
			...
			$rs[_admin]="onclick=\"alert('妳无权琯理');return false;\" style='color:#ccc;'";
		}
