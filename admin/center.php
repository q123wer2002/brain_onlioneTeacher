...
if($job=="config"&&$Apower[center_config])
{
	...
	$mgroupdb['allowrp']?$guest_reply='允许髮表評價':$guest_reply='不允许髮表評價';
	$webdb[updir]=$webdb[updir]?$webdb[updir]:$updir;

	$truepath=preg_replace("/(.+)\/([^\/]+)\/([^\/]+)/is","\\1",$WEBURL);
	if($webdb[truepath]!=$truepath){
		//echo "<CENTER>出错了!!!請正确修改程序所在目录为<font color=red>$truepath</font></CENTER><br><br><br><br>";
	}
	$webdb[truepath] || $webdb[truepath]=$truepath;
	if(!function_exists("ImageJpeg")||!function_exists("imagegif")){
		$gdmsg='提示:當前系统检测到妳的服务器不能全面的支持GD库生成水印缩略图,建议不要启用';
	}else{
		$gdmsg='提示:當前系统检测到妳的服务器基本可以支持GD库生成水印缩略图';
	}
...
{
	if(!function_exists("ImageJpeg")&&!function_exists("imagegif")){
		echo "报错提示:妳的空間没装GD库不支持图片处理功能,請必须禁用以下所有功能,否则系统运行不正常<br><br><br><br>";
	}
...
elseif($action=="config"&&($Apower[center_config]||$Apower[center_cache]||$Apower[user_reg]))
{
	if($webdb[NewsMakeHtml]==1 && $webdbs[RewriteUrl]==1){
		showmsg('妳已经选择了真静态,就不能选择伪静态,要使用伪静态,請先取消真静态!!');
	}
...
elseif($job=="map")
{
	...
	if(!strstr(preg_replace("/http:\/\/(www\.|)(.*)/is","\\2",$WEBURL),preg_replace("/http:\/\/(www\.|)(.*)/is","\\2",$webdb[www_url]))&&$webdb[www_url]!='/.'){
		...
		echo "<CENTER><A HREF='index.php?lfj=center&job=config' style='color:red;font-size:25px;'>严重警告,妳设置的“整站访问網阯”有误,請點擊重新设置</A></CENTER>";
		exit;
	}
	if(!strstr($WEBURL,$webdb[admin_url])){
		echo '<meta http-equiv="Content-Type" content="text/html; charset=gb2312" />';
		echo "<CENTER><A HREF='index.php?lfj=center&job=config' style='color:red;font-size:25px;'>严重警告,妳设置的“後台访问網阯”有误,請點擊重新设置</A></CENTER>";
		exit;
	}
	...
	foreach( $dirdb AS $key=>$value){
		...
		if(!is_writable($dirname)){
			echo '<meta http-equiv="Content-Type" content="text/html; charset=gb2312" />';
			echo "<CENTER><A style='color:red;font-size:25px;'>严重警告,".$dirname."此目录不可寫，請用FTP修改其属性为0777可寫</A></CENTER>";
			exit;
		}
		$dir=opendir($dirname);
		while($file=readdir($dir)){
			...
			if(!is_writable($_f)){
				echo '<meta http-equiv="Content-Type" content="text/html; charset=gb2312" />';
				echo "<CENTER><A style='color:red;font-size:25px;'>严重警告,".$_f."此目录不可寫，請用FTP修改其属性为0777可寫</A></CENTER>";
				exit;
			}
		}
	}
	...
	if( mysql_get_server_info() > '4.1' && $webdb[passport_type] ){
		...
		if($c1&&$c2&&($c1!=$c2)){
			echo '<meta http-equiv="Content-Type" content="text/html; charset=gb2312" />';
			echo "<CENTER><A style='color:red;font-size:25px;'>严重警告,整合论坛失败,因为妳论坛使用的数据库编碼是:$c1,而整站使用的数据库编碼是:$c2,这是不允许的，必须使整站的数据库编碼与论坛的一样，现在妳必须备份整站数据库,然後修改数据库配置文件data/mysql_config.php数据库编碼为$c1，然後再到地阯栏还原数据库选择编碼为:$c1,只能在地阯栏还原.不能在後台还原.因为後台还原不能选择数据库编碼,以上操作步骤绝对不能颠倒,即第一步整站後台备份数据,第二步修改数据库配置文件,第三步地阯栏目还原数据库</A></CENTER>";
			exit;
		}
	}
	...
	if( mysql_get_server_info() > '4.1' ){
		...
		if($c2&&$c2!=$dbcharset){
			echo '<meta http-equiv="Content-Type" content="text/html; charset=gb2312" />';
			echo "<CENTER><A style='color:red;font-size:25px;'>严重警告,妳的数据库编碼是:$c2,妳必须修改文件/data/mysql_config.php，在他倒数第二行加上这句话(注意结尾分號不能少):\$dbcharset = '$c2';</A></CENTER>";
			exit;
		}
	}
	...
	if( mysql_get_server_info() < '4.1' && $dbcharset){
		echo '<meta http-equiv="Content-Type" content="text/html; charset=gb2312" />';
		echo "<CENTER><A style='color:red;font-size:25px;'>严重警告,妳的数据库是低版本，妳必须修改文件/data/mysql_config.php，在他倒数第二行加上这句话(注意结尾分號不能少):\$dbcharset = '';</A></CENTER>";
		exit;
	}
	...
	if( eregi("^pwbbs",$webdb[passport_type]) && $webdb[passport_TogetherLogin] )
	{
		...
		{
			echo '<meta http-equiv="Content-Type" content="text/html; charset=gb2312" />';
			echo "<CENTER><A HREF='index.php?lfj=center&job=config' style='color:red;font-size:25px;'>严重警告,妳设置的“COOKIE域名”有误,請點擊重新设置,妳必须跟论坛保持一致,如果论坛设置,那么整站也要设置,如果论坛不设置,整站也不能设置,必须要保持一致.否则同步登錄会不成功</A></CENTER>";
			exit;
		}
	}
	...
	if(!strstr($files,'$OfficialNotice')&&filesize(ROOT_PATH."cache/OfficialNotice.txt")>9){
		echo "<SCRIPT LANGUAGE='JavaScript'>
			...
			function openwindows(){
				window.open('http://$OfficialUrl1$OfficialUrl2/OfficialNotice.php?E=$qibos_Edition&upgrade_Record=$webdb[upgrade_Record]');
				}
			openwindows();
			//-->
		</SCRIPT>";
