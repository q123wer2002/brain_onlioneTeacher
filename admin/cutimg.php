...
if($action=="cutimg"){
	...
	if($nextpic==1){
		...
		die("剪裁成功,妳可以點擊查看三种截图效果:<br><A HREF='$pic3' target=_blank>样式1</A> <A HREF='$pic2' target=_blank>样式2</A> <A HREF='$pic1' target=_blank>样式3</A><br> <a href='javascript:window.self.close()'>點擊关闭</a>");
	}else{
		$url="$uploadfile?$timestamp";
	}
	echo "<META HTTP-EQUIV=REFRESH CONTENT='0;URL=$url'>";
	exit;
	//die("剪裁成功,<A HREF='$uploadfile?$timestamp' target=_blank>點擊查看效果</A> <a href='javascript:window.self.close()'>點擊关闭</a>");
}
if(!ereg("^http:",$srcimg)){
	$srcimg="$weburl_array[upfiles]/$srcimg";
}
require(dirname(__FILE__)."/"."template/cutimg/cutimg.htm");
?>