...
elseif($job=="list"&&$Apower[friendlink_mod])
{
	...
	while($rs=$db->fetch_array($query)){
		...
		$rs[yz]=$rs[yz]?"<a href='index.php?lfj=$lfj&job=setyz&yz=0&id=$rs[id]' style='color:red;'><img alt='已通过审核,點擊取消审核' src='../member/images/check_yes.gif' border=0></a>":"<a href='index.php?lfj=$lfj&job=setyz&yz=1&id=$rs[id]' style='color:blue;'><img alt='还没通过审核,點擊通过审核' src='../member/images/check_no.gif' border=0></a>";
		$listdb[]=$rs;
	}
	require(dirname(__FILE__)."/"."head.php");
	require(dirname(__FILE__)."/"."template/friendlink/menu.htm");
