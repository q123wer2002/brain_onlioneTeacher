...
$ForceEnter=0;	//把0改成1将强制进入後台


//还原数据時,防止用户名被删除後,不能继续还原数据
...
if( $_POST[loginname] && $_POST[loginpwd] )
{
	if( $webdb[yzImgAdminLogin]&&$webdb[web_open] ){
		if(!check_imgnum($yzimg)){
			if(!strstr($WEBURL,$webdb[www_url])){
				echo "<CENTER>網阯有误,請重新登錄</CENTER><META HTTP-EQUIV=REFRESH CONTENT='1;URL=$webdb[admin_url]'>";
				exit;
			}
			showmsg("<A HREF=?>验证碼不符合</A>");
		}else{
			set_cookie("yzImgNum","");
		}
	}
	...
	if($rs==0){
		...
		login_logs($_POST[loginname],$_POST[loginpwd]);
		setcookie("Admin",'',0,"/");
		showmsg("<A HREF=?>密碼不正确</A>");
	}else{
		login_logs($_POST[loginname],"成功登錄,保密了");
	...
	if($userdb && mymd5($userdb[password])==$admin_pwd ){
		...
		elseif($userdb[groupid]&&file_exists(ROOT_PATH."data/group/$userdb[groupid].php"))
		{
			...
			if(!$groupdb['allowadmin']){
				...
				if(!$allowlogin){
					setcookie("Admin",'',0,"/");
					showmsg("妳當前所在用户组,系统设置无权访问整站後台,如果妳是频道琯理员,請到频道的後台登錄");
				}
			}else{
				$Apower=($groupdb[allowadmindb]);
			}
		...
		{
			setcookie("Admin",'',0,"/");
			showmsg("妳當前所在用户组,无权访问");
		}
	}else{
		setcookie("Admin",'',0,"/");
		showmsg("<A HREF='index.php?iframe=1'>請输入正确密碼帐號再访问</A>");
	}
}
...
function select_style($name='stylekey',$ck='',$url='',$select=''){
	...
	while($file=readdir($filedir)){
		if(ereg("\.php$",$file)){
			...
			$ck==$styledb[keywords]?$ckk='selected':$ckk='';	//选定某個
...
function select_template($cname,$type=1,$ck=''){
	global $db,$pre;
	$show="<select name='$cname' $reto><option value='' style='color:red;'>請选择模板</option>";
	if($type==7||$type==8){
	...
	foreach($filedb AS $word=>$array)
	{
		if($Apower[$array[power]]||is_numeric($array[power]))
		{
			if($array[power]=="label_index"){
				$show.="<tr><td onmouseover=\"this.className='leftA'\" onmouseout=\"this.className='leftB'\"><A HREF='$pathurl$array[link]' target=main>$word</A> | <A HREF='../do/index.php?&ch=1&MakeIndex=1' target='_blank' onclick=\"return confirm('妳确实要把首页生成静态吗?生成静态後,如有更改其它相关设置,需要重新點擊生成一次静态.才可以看到效果.');\">主页静态</A></td></tr>";
			}else{
...
function check_fup($table,$fid,$fup){
	...
	while($rs = $db->fetch_array($query)){
		if($rs[fup]==$fid){
			showmsg("妳不能设置本身的子栏目作为父栏目,这是不允许的.但妳可以设置其他子栏目作为父栏目");
		}elseif($rs[fup]){
...
function systemMsg(){
	...
	$rs[max_ex_time]=ini_get('max_execution_time').' 秒';	//最大执行時間
	$rs[sys_mail]= ini_get('sendmail_path') ? 'Unix Sendmail ( Path: '.ini_get('sendmail_path').')' :( ini_get('SMTP') ? 'SMTP ( Server: '.ini_get('SMTP').')': 'Disabled' );	//郵件支持模式
	$rs[systemtime]=date("Y-m-j g:i A");	//服务器所在時間
	$rs[onlineip]=$onlineip;				//當前IP
	...
	$rs[SCRIPT_FILENAME]=$_SERVER["SCRIPT_FILENAME"]?$_SERVER["SCRIPT_FILENAME"]:$_SERVER["PATH_TRANSLATED"];//當前文件路径
	...
	$rs[memory_user_limit]=ini_get('memory_limit');    //最大执行時間/空間限制内存
...
function write_config_cache($webdbs)
{
	...
	if(!is_writable(ROOT_PATH."data/config.php")){
		showmsg(ROOT_PATH."data/config.php文件目录权限不可寫,請仔细检查");
	}
}
...
function hack_admin_tpl($html_filet_ype,$getmenu=true){
	...
	$dirname=$hack?$hack:$lfj;	//hack变量优先,防止跟总後台的文件髮生冲突
