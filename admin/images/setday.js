...
document.writeln('        <span Author=meizz id=meizzYearHead onmouseover="style.backgroundColor=\'yellow\'" onmouseout="style.backgroundColor=\'white\'" title="點擊这里选择年份" onclick="tmpSelectYearInnerHTML(this.innerText)" style="cursor: hand;"></span>&nbsp;年&nbsp;<span');
document.writeln('         id=meizzMonthHead Author=meizz onmouseover="style.backgroundColor=\'yellow\'" onmouseout="style.backgroundColor=\'white\'" title="點擊这里选择月份" onclick="tmpSelectMonthInnerHTML(this.innerText)" style="cursor: hand;"></span>&nbsp;月</td>');
...
document.writeln('         Author=meizz title="返迴（不选择日期）"><u>关闭</u></span>&nbsp;</td></tr>');
...
document.writeln('             onfocus="this.blur()" title="显示當前時間" style="cursor: hand;BACKGROUND-COLOR: #808080;BORDER-BOTTOM: #808080 1px outset; BORDER-LEFT: #808080 1px outset; BORDER-RIGHT: #808080 1px outset; BORDER-TOP: #808080 1px outset;font-size: 12px; height: 20px;color: #FFFFFF; font-weight: bold"></td><td ');
...
function setday(tt,timetype) //主调函数
{
  ...
  if (arguments.length == 0){alert("对不起！您没有传迴本控件任何参数！");return;}
  var dads  = document.all.meizzDateLayer.style;
  var th = tt;
  var ttop  = tt.offsetTop;     //TT控件的定位點高
  var thei  = tt.clientHeight;  //TT控件本身的高
  var tleft = tt.offsetLeft;    //TT控件的定位點宽
...
var MonHead = new Array(12);    		   //定义阳历中每個月的最大天数
    MonHead[0] = 31; MonHead[1] = 28; MonHead[2] = 31; MonHead[3] = 30; MonHead[4]  = 31; MonHead[5]  = 30;
    MonHead[6] = 31; MonHead[7] = 31; MonHead[8] = 30; MonHead[9] = 31; MonHead[10] = 30; MonHead[11] = 31;

var meizzTheYear=new Date().getFullYear(); //定义年的变量的初始值
var meizzTheMonth=new Date().getMonth()+1; //定义月的变量的初始值
var meizzWDay=new Array(37);               //定义寫日期的数组

function document.onclick() //任意點擊時关闭该控件
{ 
...
function meizzWriteHead(yy,mm)  //往 head 中寫入當前的年与月
...
function meizzSetDay(yy,mm)   //主要的寫程序**********
{
  ...
  for (var i = 0; i < 37; i++)
  { var da = eval("document.all.meizzDay"+i)     //书寫新的一個月的日期星期排列
    if (meizzWDay[i]!="")
...
function meizzDayClick(n)  //點擊显示框选取日期，主输入函数*************
{
  var yy = meizzTheYear;
