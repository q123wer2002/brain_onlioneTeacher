...
if($inc){
	...
	elseif(ereg("^Info_",$inc)){//Info_fenlei_
		...
		if($inc=='Info_group_'){
			include(dirname(__FILE__)."/label/group.php");
		}
		elseif( is_file($label_file) ){
			include($label_file);
		}
		else{
...
function do_post(){
	...
	make_tag_cache();
	SetModule_config();
	jump("<CENTER>[<A HREF='$viewurl' target='_blank'>點擊浏览效果</A>] [<A HREF='$FROMURL'>继续修改</A>]  [<A HREF='$viewurl&jobs=show'>返迴频道/专題</A>]</CENTER>","$FROMURL",600);
}
...
<tr><td style="padding-left:20px;padding-bottom:10px;"><select id="selectTyls" onChange="selectTpl(this)">
    <option value='0'>快速选择</option><option value='1' style='color:blue;'>新建一個模板</option>$select
  </select> [<a href="#LOOK" onclick="show_MorePic(-1)">上一個</a>] 
      【<span id="upfile_PicNum">1/2</span>】[<a href="#LOOK" onclick="show_MorePic(1)">下一個</a>]  
