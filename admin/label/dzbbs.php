...
if($action=='mod'){
	...
	//选择显示两列以上,这里选择Table,否则不一定能显示效果,选择table指外套一個TABLE,选择div指不套多余的代碼
	if($colspan>1){
		$DivTpl=0;
	}else{
		$DivTpl=1;
	...
	//有辅助模板的话,要特别处理.如果仅是辅助模板有图片的话,就要读多一条数据,否则的话.都是從主模板获取.
	/*
	...
	*/
	...
	if(!$db->query("SELECT * FROM {$webdb[dzbbs_pre]}forum_forum LIMIT 1",'',0)){
		showmsg("论坛的数据表前缀不对“{$webdb[dzbbs_pre]}”，請查看论坛文件/config/config_global.php查找tablepre这一行");
	}
	$rsdb=get_label();
