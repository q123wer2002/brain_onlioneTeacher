...
if($job=="showtplpart")
{
	//$show="<select name='tplpart'  onChange='M_jumpMenu2(this,$t)'><option>請选择</option>";
	unset($show);
	...
	<!--
	parent.showtype(\"<select name='tplpart'  onChange='M_jumpMenu2(this,1)'><option>請选择</option><option style='color:red;' value='-1' >新建一個模板</option>$show</select>\",1);
	parent.showtype(\"<select name='tplpart'  onChange='M_jumpMenu2(this,2)'><option>請选择</option><option style='color:red;' value='-1' >新建一個模板</option>$show</select>\",2);
	//-->
	</SCRIPT>";
	exit;
}
	...
	*/
	...
	if($tplpart_1)
	{
		...
		if(!$postdb[tplpart_1code]){
			showmsg("模板一路径不对或者是其他原因,模板数据读取失败,請检查之");
		}
	}
	if($tplpart_2)
	{
		...
		if(!$postdb[tplpart_2code]){
			showmsg("模板二路径不对或者是其他原因,模板数据读取失败,請检查之");
		}
	}
