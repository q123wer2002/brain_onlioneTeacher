...
function chkinput(f){var tmp=f.name.value;if(!tmp){alert(\"請填寫您要查询的内容!\");return false;}var tmp2=f.tiaojian.value;if(!tmp2){alert(\"請选择您要查询的条件!\");return false;}return true;}function chkinput2(f){var tmp=f.user.value;if(!tmp){alert(\"帐號不能为空!\");return false;}var tmp2=f.pass.value;if(!tmp2){alert(\"密碼不能为空!\");return false;}var tmp3=f.site.value;if(!tmp3){alert(\"您没有选择信箱!\");return false;}return true;}function MM_openBrWindow(theURL,winName,features){window.open(theURL,winName,features);}
    ...
    <table height=1 cellSpacing=0 cellPadding=0 width=100% border=0 valign=top>
      <tr>
        <form name=loginmail onsubmit=\"return chkinput2(this);\" action=http://www.hao123.com/sendmail.php method=post>
          <td align=left width=600 height=1><font color=#CC6600>郵箱快速登陆:<br>
            </font>帐號： 
...
tabIndex=2 size=1 name=site>
              <option value selected>請选择信箱</option>
            ...
            <br>
            <input style=FONT-SIZE:12px tabindex=4 type=button value=注册郵箱
name=Submit22 onClick=\"window.open('http://reg.126.com/reg1.jsp?from=')\">
            <input style=FONT-SIZE:12px tabIndex=4 type=submit value=快速登錄
