...
if($job=="list"&&$Apower[member_list])
{
	...
	while($rs=$db->fetch_array($query)){
		...
		if(!$rs[groupid]){
			$rs[alert]="alert('此用户的资料,还没有在整站激活,妳不能进行任何操作!');return false;";
		}else{
			$rs[alert]="";
		}

		if($rs[yz]){
			$rs[yz]="<A HREF='index.php?lfj=$lfj&action=yz&uid_db[0]=$rs[uid]&T=noyz' style='color:red;' onclick=\"$rs[alert]\" title='已经通过审核,點擊即可取消审核'><img src='../member/images/check_yes.gif' border='0'></A>";
		}elseif($rs[groupid]){
			$rs[yz]="<A HREF='index.php?lfj=$lfj&action=yz&uid_db[0]=$rs[uid]&T=yz' style='color:blue;' onclick=\"$rs[alert]\" title='还没有通过审核,點擊即可通过审核'><img src='../member/images/check_no.gif' border='0'></A>";
		}else{
			$rs[yz]="未激活";
		}
...
elseif($action=="editmember"&&$Apower[member_list])
{
	...
	if($memberlevel[$postdb[groupid]]){
		if(!$webdb[groupUpType]&&$postdb[money]<$memberlevel[$postdb[groupid]]){
			showmsg("{$ltitle[$postdb[groupid]]}是会员组,而當前用户的积分不足{$memberlevel[$postdb[groupid]]},所以妳不能设置为此组");
		}elseif($webdb[groupUpType]&&!$ConfigDB[endtime]){
			showmsg("{$ltitle[$postdb[groupid]]}是会员组,妳必须为他指定有效日期!");
		}
	}
	
	if($rsdb[groupid]=='3'&&$userdb[groupid]!=3&&!$founder&&!$ForceEnter){
		showmsg("妳无权限修改超级琯理员用户组");
	...
	add_user( $uid , ($postdb[money]-$rsdb[money]) ,'琯理员操作');

	//自定义用户字段
	Reg_memberdata_field($uid,$postdb);

	jump("脩改成功","index.php?lfj=member&job=editmember&uid=$uid");
