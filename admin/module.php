...
elseif($action=='automake')
{
	...
	//以下这段是处理原系统中存在id值雷同的情况,要特别处理qb_module,qb_label,qb_*_config这3個表
...
elseif($action=='mod')
{
	...
	if(!is_dir(ROOT_PATH."$postdb[dirname]/$postdb[admindir]")){
		showerr("後台目录不存在".ROOT_PATH."$postdb[dirname]/$postdb[admindir]");
	}
	if(!is_writable(ROOT_PATH."$postdb[dirname]/data/")){
		showerr(ROOT_PATH."$postdb[dirname]/data/"."目录不可寫");
	}
	...
	if(!is_table($table)){
		jump("脩改成功!","index.php?lfj=module&job=list",1);
	}

	if(!is_writable(ROOT_PATH."$postdb[dirname]/data/config.php")){
		showerr(ROOT_PATH."$postdb[dirname]/data/config.php"."文件不可寫");
	}
...
elseif($action=="del")
{
	...
	$db->query("DELETE FROM `{$pre}label` WHERE module='$id'");	//删除標签
...
elseif($action=='copy')
{
	...
	jump("复制成功,請设置一下新模块的後台权限","index.php?lfj=group&job=admin_gr&gid=3",10);
}
elseif($action=="order")
{
