...
elseif($job=='into'&&$Apower[mysql_into]){
	...
	if(file_exists(ROOT_PATH."cache/mysql_insert.txt")){
		echo "<CENTER><table><tr bgcolor=#FF0000><td colspan=5 height=30><div align=center><A HREF=".read_file(ROOT_PATH."cache/mysql_insert.txt")."><b><font color=ffffff>上次还原数据被中断是否继续,點擊继续</font></b></A></div></td></tr></table></CENTER>";
	}
	require(dirname(__FILE__)."/"."head.php");
	require(dirname(__FILE__)."/"."template/mysql/menu.htm");
...
elseif($action=='del'&&$Apower[mysql_del]){
	...
	if(!is_dir(ROOT_PATH."cache/mysql_bak/$baktime")){
		...
		jump("数据删除失败,請确认目录属性为0777","index.php?lfj=mysql&job=del",5);
	}
}
...
elseif($action=='sql'&&$Apower[mysql_sql]){
	...
	jump("如果網页上方没出现乱碼，则操作成功","index.php?lfj=mysql&job=sql",10);
...
function bak_out($tabledb){
	...
	if(!$rand_dir){
		...
		if( file_exists(ROOT_PATH."cache/mysql_bak/mysql") )
		{
			...
			{
				showmsg(ROOT_PATH."cache/mysql_bak/$rand_dir,目录不可寫,請改属性为0777");
			}
			//复制一個自动还原的文件到SQL目录.方便日後还原
	...
	//當前表能取到数据時,继续此表下一页取数据,否则從下一個表的0开始
	...
	if($tabledb[$tableid])
	{
		...
		//記录下來.防止中途备份失败
		write_file(ROOT_PATH."cache/bak_mysql.txt",mymd5("index.php?lfj=$lfj&action=out&page=$page&rowsnum=$rowsnum&tableid=$tableid&rand_dir=$rand_dir&step=$step&tabledbreto=$Table&baksize=$baksize"));

		echo "<CENTER>已备份 <font color=red>$step</font> 卷, 进度条 <font color=blue>{$page}</font> 當前正在备份数据库 <font color=red>$tabledb[$tableid]</font></CENTER>";
...
EOT;
	...
	else
	{
		...
		if( !@is_writable(ROOT_PATH."cache/mysql_bak/$rand_dir/0.sql") ){
			showmsg("备份失败，請在cache/mysql_bak/目录下创建一個目录mysql然後改其属性为0777,如果此目录已存在，請删除他，重新创建，并改属性为0777");
		}
		return $rs;
	}
}
...
function bak_time(){
	$show="<select  name='baktime'><option value='' selected>請选择备份文件</option>";
