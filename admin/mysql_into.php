...
if(!@mysql_connect($dbhost, $dbuser, $dbpw)) {
	die('MYSQL 连接数据库失败,請确定数据库用户名,密碼设置正确<br><A HREF="#" onclick="history.back(-1)">點擊返迴</A>');
}
if(!@mysql_select_db($dbname)){
	die("MYSQL 连接成功,但當前使用的数据库 {$dbname} 不存在<br><A HREF=\"#\" onclick=\"history.back(-1)\">點擊返迴</A>");
}
...
if(is_file("$page.sql")){
	...
	$page++;
	echo "正在导入第{$page}卷,請稍候...<META HTTP-EQUIV=REFRESH CONTENT='0;URL=?page=$page'>";
	exit;
}else{
	echo "数据导入完毕,如果妳是從旧空間转移数据到新空間,請把旧空間/data/目录下的全部文件移到新空間对应的/data/目录里.但mysql_config.php不必移.因为新的数据库配置与旧的一般不相同,因而妳需要手工配置好新空間的这個文件mysql_config.php";
}
