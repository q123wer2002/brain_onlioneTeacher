...
while($rs = $db->fetch_array($query))
{
	...
	if($timestamp-$rs[posttime]<(3600*$webdb[UpdatePostTime])){
		$rs[update]='<A HREF="#" style="color:#ccc;" onclick="alert(\'距离上次更新時間'.$webdb[UpdatePostTime].'小時後,才可以进行刷新!\')">刷新</A>';
	}else{
	...
	$rs[pop1]="<A HREF=\"$webdb[www_url]/job.php?job=popshow&type=1&fid=$rs[fid]&id=$rs[id]\">首页焦點</A>";	
	$rs[pop2]="<A HREF=\"$webdb[www_url]/job.php?job=popshow&type=2&fid=$rs[fid]&id=$rs[id]\">栏目焦點</A>";
	$rs[pop3]="<A HREF=\"$webdb[www_url]/job.php?job=popshow&type=3&fid=$rs[fid]&id=$rs[id]\">分类焦點</A>";
	$query2 = $db->query("SELECT * FROM `{$_pre}buyad` WHERE `id`='$rs[id]' AND `endtime`>'$timestamp'");
	while($rs2 = $db->fetch_array($query2)){
		if($rs2[sortid]==-1){
			$rs[pop1]='<A HREF="#" style="color:#ccc;" onclick="alert(\'已经首页焦點显示了\')">首页焦點</A>';
		}elseif($rs2[sortid]==$rs[fid]){
			$rs[pop3]='<A HREF="#" style="color:#ccc;" onclick="alert(\'已经栏目焦點显示了\')">分类焦點</A>';
		}elseif($rs2[sortid]){
			$rs[pop2]='<A HREF="#" style="color:#ccc;" onclick="alert(\'已经分类焦點显示了\')">栏目焦點</A>';
		}
	}
