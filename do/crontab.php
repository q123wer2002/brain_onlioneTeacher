...
if(time()-@filemtime($filename)>$wait_time){	//如果某段時間内没被修改过,那就可能線程已死掉

	require_once(dirname(__FILE__)."/global.php");
	$cdo = true;
	do{
		if(!touch($filename)){
			die('文件不可寫/cache/crontab.php');
		}
		$cdo = false;
		$times = time();
		...
		while($rs = $db->fetch_array($query)){
			...
			}elseif($rs[daytime]){	//每天执行一次,時+分格式如 1403
			...
			}elseif($rs[whiletime]){	//未來某個時間的,只执行一次
