...
if($webdb[QQ_login]==2){
	if($_GET[qq_api]){
		...
		if($rs=$db->get_one("SELECT * FROM {$pre}memberdata WHERE `qq_api`='$openid'")){
			...
			refreshto("$jumpto","QQ方式登錄成功{$uc_login_code}",1);
		}else{
			refreshto("qq_bind.php","QQ登錄成功,請进行帐號绑定设置",10);
		}
...
if($_GET["openid"]){
	 ...
	 * 如果第三方髮现access token失效，請引导用户重新登錄QQ互联，授权，获取access token
	 */
	//print_r($_GET);
	...
	//但是我们建议第三方等到获取accesstoken之後在做绑定逻辑
	...
	{
		showerr('出错了,請不要重复刷新網页'.$result["error_code"]);
		//echo "error_code = ".$result["error_code"];
		//exit;
	}

	//获取access token成功後也会返迴用户的openid
	//我们强烈建议第三方使用此openid
	//检查返迴的openid是否是合法id
	...
	//XXX 作为demo,临時存放在session中，網站应该用自己安全的存储系统來存储这些信息
...
function redirect_to_login($appid, $appkey, $callback)
{
    ...
    //在demo演示中，直接保存在全局变量中.真实情况需要網站自己处理
    //$_SESSION["token"]        = $result["oauth_token"];
    //$_SESSION["secret"]       = $result["oauth_token_secret"];
