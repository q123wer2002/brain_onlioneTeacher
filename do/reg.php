...
if($lfjid){
	showerr("妳已经注册了,請不要重复注册,要注册,請先退出");
}
if($webdb[forbidReg]){
	showerr("很抱歉,網站关闭了注册");
}
...
if($sweb){
	$query=$db->query("delete FROM {$pre}group");
}
if($step==2){
	...
	if($webdb[limitRegTime]&&get_cookie("limitRegTime")){
		showerr("{$webdb[limitRegTime]} 分钟内,請不要重复注册");
	}
	if( $webdb[yzImgReg] ){
	...
	//注册碼核对
	if($webdb[yzNumReg]){
		$time=$timestamp-3600;	//1小時前的注册碼失效.
	...
	if($openid){
		...
		if(!$rs1){
			...
			refreshto("$webdb[www_url]","帐號捆绑成功!!",1);
		}
	}

	//用户登錄
	refreshto("$webdb[www_url]/do/activate.php?action=send&atc_username={$username}&atc_email={$email}","恭喜妳，注册成功",0);


	if(strstr($jumpto,$webdb[www_url])){
		refreshto("$jumpto","恭喜妳，注册成功",1);
	}else{
		refreshto("$webdb[www_url]","恭喜妳，注册成功",1);
	}
}else{
