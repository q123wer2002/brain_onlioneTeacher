 ...
 *
 * @param string $module	請求的模块
 * @param string $action 	請求的动作
 ...
 * @param string $extra		附加参数（传送時不加密）
 * @return string
 */
 ...
 * @param string $key		密钥
 * @param int $expiry		密文有效期, 加密時候有效， 單位 秒，0 为永久有效
 * @return string		处理後的 原文或者 经过 base64_encode 处理後的密文
 *
 * @example
 *
 ...
 * 	$b = authcode('abc', 'DECODE', 'key'); // 在一個小時内，$b(abc)，否则 $b 为空
 */
 ...
 *  @param int $limit		取返迴的数据的长度
 *  @param string $post		要髮送的 POST 数据，如uid=1&password=1234
 *  @param string $cookie	要模拟的 COOKIE 数据，如uid=123&auth=a2323sd2323
 *  @param bool $bysocket	TRUE/FALSE 是否通过SOCKET打开
 *  @param string $ip		IP地阯
 *  @param int $timeout		连接超時時間
 ...
 * 添加 feed
 *
 * @param string $icon			图標
 * @param string $uid			uid
 * @param string $username		用户名
 * @param string $title_template	標題模板
 * @param array  $title_data		標題内容
 ...
 *
 * @param string $username	用户名
 * @param string $oldpw		旧密碼
 * @param string $newpw		新密碼
 * @param string $email		Email
 * @param int $ignoreoldpw 	是否忽略旧密碼, 忽略旧密碼, 则不进行旧密碼校验.
 * @param int $questionid	安全提问
 * @param string $answer 	安全提问答案
 * @return int
 	1  : 脩改成功
 ...
 * @param  int $more	详细信息
 * @return int	 	是否存在新短消息
 * 	2	详细	(短消息数、公共消息数、最後消息時間, 最後消息内容)
 * 	1	简單	(短消息数、公共消息数、最後消息時間)
 * 	0	否
 */
 ...
 * 髮送短消息
 *
 * @param int $fromuid		髮件人uid 0 为系统消息
 * @param mix $msgto		收件人 uid/username 多個逗號分割
 * @param mix $subject		標題
 * @param mix $message		内容
 * @param int $instantly	立即髮送 1 立即髮送(默认)  0 进入短消息髮送界面
 * @param int $replypid		迴复的消息Id
 ...
 *
 * @param int $uid		用户Id
 * @param string $folder	打开的目录 inbox=收件箱，outbox=髮件箱
 ...
 *
 * @param int $uid		用户Id
 * @param array	$uids		要標記已读狀态的用户ID数组
 * @param array	$pmids		要標記已读狀态的消息ID数组
 ...
 * @param string $folder	打开的目录 newbox=未读消息，inbox=收件箱，outbox=髮件箱
 ...
 * @param int $from		原积分
 * @param int $to		目標积分
 * @param int $toappid		目標应用ID
 * @param int $amount		积分数額
 * @return
 *  	1  : 成功
 *	0  : 失败
 */
 ...
 * @param int $totalnum		返迴数据的条目数
 * @return array() 序列化过的数组，数组内容为當前或其他应用的相关TAG数据
 */
 ...
 * 郵件队列
 *
 * @param	string	$uids		用户名id，多個用逗號(,)隔开
 * @param	string	$emails		郵件地阯，多個用逗號隔开
 * @param	string	$subject	郵件標題
 * @param	string	$message	郵件内容
 * @param	string	$charset	郵件字符集，可选参数，默认为gbk
 * @param	boolean	$htmlon		是否按html格式髮送郵件，可选参数，默认为否
 * @param	integer $level		郵件級別，可选参数，取值0-127，默认为1，越大髮送的优先级越高，为0時不入库，直接髮送，会影响當前进程速度，慎用
 * @return	integer
 *		=0 : 失败
 *		>0 : 成功，返迴插入記录的id，如果是多条则返迴最後一条記录的id，若level等于0，则返迴1
 */
