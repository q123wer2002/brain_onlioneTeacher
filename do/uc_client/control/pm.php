...
define('PMLIMIT1DAY_ERROR', -1);//note 超出了24小時最大允许髮送短消息数目
define('PMFLOODCTRL_ERROR', -2);//note 不满足两次髮送短消息最短間隔
define('PMMSGTONOTFRIEND', -3);//note 群髮短消息，但是目標并非髮信者好友
define('PMSENDREGDAYS', -4);//note 髮短消息最少注册天数
...
class pmcontrol extends base {
	...
	function onsendpm() {
		...
		if($fromuid) {
			...
			$this->load('friend');
			if(count($msgto) > 1 && !($is_friend = $_ENV['friend']->is_friend($fromuid, $msgto, 3))) {//note 如果有群髮但是目標并非髮信人的好友则不允许髮送
