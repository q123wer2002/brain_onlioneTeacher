...
define('UC_NOTE_TIMEOUT', 15);	//note 通知超時時間(秒)
define('UC_NOTE_GC', 10000);	//note 过期通知的迴收概率，该值越大，概率越低
...
class notemodel {
	...
	function _send() {
		...
		//note 如果内存表記录不存在，那么可能 mysql 被重启，需要再次判断通知是否存在
	...
	function sendone($appid, $noteid = 0, $note = '') {
		...
		$returnsucceed = $response != '' && ($response == 1 || is_array(xml_unserialize($response)));//note 當确实返迴为1的時候才认为是通知成功
