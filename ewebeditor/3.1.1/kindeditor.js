...
KE.lang = {
    ...
    justifyright : '右对齐',
    justifyfull : '两端对齐',
    insertorderedlist : '编號',
    insertunorderedlist : '项目符號',
    indent : '增加缩进',
    outdent : '减少缩进',
    subscript : '下標',
    superscript : '上標',
    ...
    invalidSwf : "請输入有效的URL地阯。\n只允许swf格式。",
	invalidRar : "請输入有效的URL地阯。",
    invalidImg : "請输入有效的URL地阯。\n只允许jpg,gif,bmp,png格式。",
    invalidMedia : "請输入有效的URL地阯。\n只允许mp3,wav,wma,wmv,mid,avi,mpg,mpeg,asf,rm,rmvb,ra,flv格式。",
    invalidMedia_url : "系统不能自动识别此媒体網阯,請必须选择一個播放器",
	invalidWidth : "宽度必须为数字。",
    invalidHeight : "高度必须为数字。",
    invalidBorder : "边框必须为数字。",
    invalidUrl : "URL不正确。",
    pleaseInput : "<font style='color:red;'>以下为引用的内容：</font> &nbsp;<br>"
};
...
KE.init = function(config) {
    ...
    var defaultItems = [
        'source', 'preview', 'fullscreen', 'undo', 'redo', 'emoticons', 'cut', 'copy', 'paste',
	...
	if(KE.handpage==true)defaultItems[defaultItems.length]='page';
    ...
    var defaultHtmlTags = {
        ...
        'a' : ['href', 'target', '.color', 'p8name'],
        'embed' : ['src', 'type', 'loop', 'autostart', 'quality', 'width', 'height', '.width', '.height', '/'],
...
KE.plugin['page'] = {
    click : function(id) {
        KE.util.selection(id);
        KE.util.select(id);
        KE.util.insertHtml(id, '<br><br>[-page-]');
        KE.util.focus(id);
    }
};
KE.plugin['preview'] = {
...
KE.plugin['flash'] = {
    ...
    check : function(id) {
        ...
        if (type == 1&&url.match(/\.(swf)$/i) == null) {
    ...
    insert : function(id, url, title, width, height, border) {
		html="<br><p align=center>[flash,"+width+","+height+"]"+url+"[/flash]</p><br>";
        KE.util.insertHtml(id, html);
        KE.layout.hide(id);
        KE.util.focus(id);
    }
...
KE.plugin['rar'] = {
    ...
    insert : function(id, url, title, width, height, border) {
        var html = '<div><a style="COLOR: red" href="'+url+'" target=_blank p8name="p8download">點擊下载</a></div>';
        KE.util.insertHtml(id, html);
        KE.layout.hide(id);
        KE.util.focus(id);
    }
...
KE.plugin['media'] = {
    ...
    check : function(id) {
        ...
        var height = KE.$('imgHeight', dialogDoc).value;
        var border = KE.$('imgBorder', dialogDoc).value;
		var playType = KE.$('play_Type', dialogDoc).value;
        if (url.match(/\.(mid|mp3|wma|wav|asf|aac|flac|ape|avi|avi|asf|asx|wmv|mpg|mpeg|ra|rm|ram|rmvb|flv)$/i) == null) {
            if (type == 1)
            {
				alert(KE.lang['invalidMedia']+url);
				window.focus();
				KE.g[id].yesButton.focus();
				return false;
            }
			...
			{
				alert(KE.lang['invalidMedia_url']);
				window.focus();
				KE.g[id].yesButton.focus();
				return false;
			}
    ...
    exec : function(id) {
        ...
        if (type == 1) {
            ...
            var border = KE.$('imgBorder', dialogDoc).value;
			var playType = KE.$('play_Type', dialogDoc).value;
            this.insert(id, url, title, width, height, border,playType);
        }
    },
    insert : function(id, url, title, width, height, border,playType) {
		mp3_RE=/\.(mid|mp3|wma|wav|asf|aac|flac|ape)$/i
		avi_RE=/\.(avi|avi|asf|asx|wmv|mpg|mpeg)$/i
		RM_RE=/\.(ra|rm|ram|rmvb)$/i
		FLV_RE=/\.(flv)$/i			
		if (RM_RE.test(url)||playType=='rm')
		{
			html="<br><p align=center>[rmvb,"+width+","+height+"]"+url+"[/rmvb]</p><br>";
		}
		else if (avi_RE.test(url)||playType=='wmv')
		{
			html="<br><p align=center>[wmv,"+width+","+height+"]"+url+"[/wmv]</p><br>";
		}
		else if (FLV_RE.test(url)||playType=='flv')
		{
			html="<br><p align=center>[flv,"+width+","+height+"]"+url+"[/flv]</p><br>";
		}
		else if (mp3_RE.test(url)||playType=='mp3')
		{
			html="<br><p align=center>[mp3,"+width+",70]"+url+"[/mp3]</p><br>";
		}
		else
		{
			html='==';
...
KE.plugin['image'] = {
    ...
    check : function(id) {
        ...
        var height = KE.$('imgHeight', dialogDoc).value;
        var border = KE.$('imgBorder', dialogDoc).value;
        if (type == 1&&url.match(/\.(jpg|jpeg|gif|bmp|png)$/i) == null) {
...
KE.plugin['layer'] = {
    click : function(id) {
        var cmd = 'layer';
        var styles = [
            'margin:5px;border:1px solid #BEBEBE;background:#FDFDDF;',
            'margin:5px;border:2px solid #BEBEBE;background:#FDFDDF;',
            'margin:5px;border:1px dashed #BEBEBE;background:#FDFDDF;',
            'margin:5px;border:2px dashed #BEBEBE;background:#FDFDDF;',
            'margin:5px;border:1px dotted #BEBEBE;background:#FDFDDF;',
            'margin:5px;border:2px dotted #BEBEBE;background:#FDFDDF;'
        ];
        KE.util.selection(id);
        var menu = new KE.menu({
            id : id,
