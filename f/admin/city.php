...
$linkdb=array("地域/省份琯理"=>"$admin_path&job=listsort","城市琯理"=>"$admin_path&job=city","城市辖区琯理"=>"$admin_path&job=zone","地段琯理"=>"$admin_path&job=street","批量操作"=>"$admin_path&job=batch");
...
elseif($action=="addstreet")
{
	...
	if(!$rs){
		refreshto("$admin_path&job=zone","請选择一個城市辖区",3);
	}
...
elseif($action=="batch")
{
	...
	if($type=="zone_dir"){
		...
		if($ckk){
			$page++;
			echo "請稍候$page....<META HTTP-EQUIV=REFRESH CONTENT='0;URL=$admin_path&action=$action&type=$type&domain2=$domain2&citydir=$citydir&page=$page&replaceold=$replaceold'>";
			exit;
		}else{
			area_cache_guide();
		}		
...
function select_fup($table,$name='fup',$ck=''){
	...
	return "<select name='$name'><option value=''>請选择</option>$show</select>";
}
...
function write_city_file($dirname,$cityid){
	...
	if(!is_dir(ROOT_PATH.$dirname)){
		showerr(ROOT_PATH.$dirname."目录创建失败,請确认分类目录权限可寫");
	}
	if( eregi("^\.\.",$dirname) ){
