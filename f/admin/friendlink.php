...
elseif($job=="list")
{
	...
	$min=($page-1)*$rows;
	$showpage=getpage("`{$_pre}friendlink` A","$SQL","?lfj=$lfj&job=$job&cityid=$cityid",$rows);
	...
	while($rs=$db->fetch_array($query)){
		...
		$rs[yz]=$rs[yz]?"<a href='$admin_path&job=setyz&yz=0&id=$rs[id]' style='color:red;'><img alt='已通过审核,點擊取消审核' src='../member/images/check_yes.gif' border=0></a>":"<a href='$admin_path&job=setyz&yz=1&id=$rs[id]' style='color:blue;'><img alt='还没通过审核,點擊通过审核' src='../member/images/check_no.gif' border=0></a>";
		$listdb[]=$rs;
	}
