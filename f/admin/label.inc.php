...
if($action=='mod'){
	...
	require(ROOT_PATH."data/all_fid.php");
	...
	$cityId='$GLOBALS[city_id]';	//限制只能显示當前城市的信息
	if($cityId){
	...
	if(count($sql_DB)>1){
		...
		if($NUM>10000){
			showerr("大于指定信息量時,妳只能选择同类分表的栏目,不同类分表的栏目不允许同時调用,否则极其影响速度");
		}
	}
	...
	if( is_table("{$_pre}module") ){
		$select_module="<select name='moduleid'><option value=''>請选择</option>";
