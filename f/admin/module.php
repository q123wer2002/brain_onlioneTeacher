...
$linkdb["模块琯理"]="$admin_path&job=listsort";
if($id){
...
if($job=="listsort")
{
	...
	$selectsortid="<select name='sort_id'><option value=''>請选择</option>";
...
elseif($action=="editorder")
{
	...
	write_module_cache();
	$Module_db->make_template($id);
	refreshto("$admin_path&job=editsort&id=$id","脩改成功",10);
}
...
elseif($action=="editfield")
{
	...
	refreshto("$admin_path&job=editsort&id=$id","脩改成功",1);
}
elseif($job=="addfield")
{
	$rsdb = $Module_db->module_info($id);
...
elseif($action=="editmodule")
{
	...
	foreach($tpldb  AS $key=>$value){
		if($value&&!is_file(Mpath.$value)&&!is_file(Mpath."template/default/$value")&&!is_file(Mpath."template/$webdb[style]/$value")){
