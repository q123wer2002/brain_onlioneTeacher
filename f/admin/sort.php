...
if($job=="listsort")
{
	...
	if(is_file(ROOT_PATH."cache/table.txt")){
		...
		die("注意:<A HREF='$url'>上一次分表中途失败,請點擊继续</A>");
	}
...
elseif($job=="editsort")
{
	...
	$_module="<select name='postdb[mid]'><option value=''>請选择所属模型</option>";
...
elseif($action=="editsort")
{
	...
	if($postdb[mid]!=$rs_fid[mid]&&$db->get_one(" SELECT * FROM {$_pre}content WHERE fid='$postdb[fid]' limit 1 ")){
		 showerr("當前栏目已经有内容了,妳要修改成其他模型的话,請先删除本栏目里的内容或把内容移走");
	}
...
elseif($action=="delete")
{
	...
	foreach( $fiddb AS $fid=>$value){
		...
		if($_rs){
			showerr("分类有子栏目妳不能删除,請先删除或移走子栏目,再删除分类");
		}
...
elseif($action=="editfield")
{
	...
	$config=addslashes(serialize($array));
	$db->query("UPDATE {$_pre}sort SET config='$config' WHERE fid='$fid' ");
	refreshto("$admin_path&job=editfield&fid=$fid&field_name=$postdb[field_name]","脩改成功",10);
}
...
elseif($action=="editorder")
{
	...
	$config=addslashes(serialize($array));
	$db->query("UPDATE {$_pre}sort SET config='$config' WHERE fid='$fid' ");
	refreshto("$admin_path&job=listfield&fid=$fid","脩改成功",10);
}
...
elseif($action=='table')
{
	...
	if($ckk){
		...
		echo "請稍候...$page<META HTTP-EQUIV=REFRESH CONTENT='0;URL=$admin_path&action=$action&tableid=$tableid&tableid0=$tableid0&fid=$fid&page=$page'>";
		exit;
	}else{
