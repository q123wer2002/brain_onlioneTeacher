...
if(!$lfjid)
{
	die("<A HREF='$webdb[www_url]/login.php' onclick=\"clickEdit.cancel('clickEdit_$TagId')\">請先登錄</A>");
}
if($atc=="do"){
	...
	if($step==2){
		...
		echo "<A HREF=\"$city_url/post.php?job=edit&fid=$fid&id=$id\">修改</A><br><A HREF=\"$city_url/post.php?action=del&fid=$fid&id=$id\" onclick=\"return confirm('妳确认要删除吗?');\">删除</A><br>";
