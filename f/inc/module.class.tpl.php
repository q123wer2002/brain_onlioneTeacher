...
function post_tpl($rs,&$tplcode){
	...
	//如果模板中存在这個变量INPUT元素,就不需要再寫进去,否则会重复
	...
	if($rs[form_type]=='text')
	{
		...
		$show0="{$rs[title]}:$mustfill";
		$show=" <input $ifmust $jsck type='text' name='postdb[{$rs[field_name]}]' id='atc_{$rs[field_name]}' class='text' value='\$rsdb[{$rs[field_name]}]'> $rs[form_units]<br>{$rs[form_title]}";
	}
	//联级字段
	elseif($rs[form_type]=='classdb')
	{
		...
		$show="<SCRIPT LANGUAGE='JavaScript'>
					...
					</SCRIPT><br>{$rs[form_title]}";
	}
	//日历选择框
	elseif($rs[form_type]=='time')
	{
		$show0="{$rs[title]}:$mustfill";
		$show="<input  onclick=\"setday(this,1)\" type='text' name='postdb[{$rs[field_name]}]' id='atc_{$rs[field_name]}' size='20' value='\$rsdb[{$rs[field_name]}]'><br>{$rs[form_title]} ";
	}
	//單图片上传
	elseif( $rs[form_type]=='onepic' )
	{
		$show0="{$rs[title]}:$mustfill";
		$show="<input type='text' name='postdb[{$rs[field_name]}]' id='atc_{$rs[field_name]}' size='50' value='\$rsdb[{$rs[field_name]}]'> $rs[form_units]<br><iframe frameborder=0 height=23 scrolling=no src='\$webdb[www_url]/do/upfile.php?fn=upfile&dir=\$_pre/\$fid&label=atc_{$rs[field_name]}' width=310></iframe><br>{$rs[form_title]}";
	}
	//單文件上传
	...
	elseif($rs[form_type]=='textarea')
	{
		...
		$show0="{$rs[title]}:$mustfill";
		$show="<textarea $ifmust name='postdb[{$rs[field_name]}]' id='atc_{$rs[field_name]}' style='width:{$rs[field_inputwidth]}px;height:{$rs[field_inputheight]}px;'>\$rsdb[{$rs[field_name]}]</textarea><br>{$rs[form_title]}";
	}
	//可视化编辑器
...
</script><textarea name='postdb[{$rs[field_name]}]' id='atc_{$rs[field_name]}' style='width:{$rs[field_inputwidth]}px;height:{$rs[field_inputheight]}px;visibility:hidden;'>\$rsdb[{$rs[field_name]}]</textarea><br>{$rs[form_title]}";
	}
...
<script type='text/javascript'>
	...
	elseif($rs[form_type]=='select')
	{
		...
		$show0="{$rs[title]}:$mustfill";
		$show="<select name='postdb[{$rs[field_name]}]' id='atc_{$rs[field_name]}'>$_show</select>$rs[form_units]<br>{$rs[form_title]}";
	}
...
\$job=='postnew' && \$num=0;\r\nfor( \$i=0; \$i<\$num ;\$i++ ){print <<<EOT\r\n-->
 ...
 地阯: 	
                    ...
                    [<a href='javascript:' onClick='window.open(\"\$webdb[www_url]/do/upfile.php?fn=upfile_{$rs[field_name]}&dir=\$_pre\$fid&label=\$i\",\"\",\"width=350,height=50,top=200,left=400\")'><font color=\"#FF0000\">點擊更换图片</font></a>] 	[<A HREF=\"javascript:delpic('\$i')\">移除</A>]
                    <br></span><!--\r\nEOT;\r\n}print <<<EOT\r\n-->
...
function listfilter_tpl($rs,$field_db,&$tplcode)
{
	...
	$show="<!--\r\nEOT;\r\n\$url=get_info_url('',\$fid,\$city_id,\$zone_id,\$street_id,array($TempSearch_array));\r\nprint <<<EOT\r\n--><A HREF='\$url&dates=\$dates&orders=\$orders' {\$search_fieldDB[{$rs[field_name]}][0]}>不限</A> ";
...
function show_tpl($rs,&$tplcode){
	...
	if($rs[form_type]=='onepic'){
		...
		$show="<A HREF='{\$rsdb[{$rs[field_name]}]}' target=_blank>點擊下载</A>";
...
function showdowns($rs){
	return "<!--{foreach \$rsdb[{$rs[field_name]}][url]  \$key \$value}--><!--{eval \$title=\$rsdb['{$rs[field_name]}'][title][\$key]?\$rsdb['{$rs[field_name]}'][title][\$key]:'點擊下载';}--><A HREF='\$value' target=_blank>\$title</A><br><!--{/foreach}-->";
}
