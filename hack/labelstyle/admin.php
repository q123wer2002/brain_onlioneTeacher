...
if($job == "out" && $Apower[labelstyle]){
	...
	while($file=readdir($filedir)){
		if(ereg("\.php$",$file)){
			...
			$show.="<div class=\"liststyles\"><span>风格名称:<em>".$styledb[name]."</em></span><a href=\"index.php?lfj=$lfj&action=out&style=".$styledb[keywords]."\">點擊导出</a></div>\n";
		}
	}
	hack_admin_tpl('labelstyle');
	exit;
...
elseif($action=='sql' && $Apower[labelstyle]){
	...
	jump("如果網页上方没出现乱碼，则操作成功","index.php?lfj=$lfj&job=in",10);	
}
