...
\$Content='$Content';
	...
	else
	{
		...
		jump("郵件髮送完毕,髮送成功的郵件有 <font color=red>{$succeeNUM}</font> 封,髮送失败的郵件有 <font color=red>{$failNUM}</font> 封","index.php?lfj=mail&job=send",30);	
	}
}
...
\$Content='$Content';
	...
	else
	{
		...
		$succeeNUM=intval($succeeNUM);
		$failNUM=intval($failNUM);
		jump("郵件髮送完毕,髮送成功的郵件有 <font color=red>{$succeeNUM}</font> 封,髮送失败的郵件有 <font color=red>{$failNUM}</font> 封","index.php?lfj=mail&job=$action",30);	
	}
}
?>