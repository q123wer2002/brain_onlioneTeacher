...
elseif($action=="send"&&$Apower[mail_send])
{
	...
	else
	{
		...
		jump("手机短信髮送完毕,髮送成功的短信有 <font color=red>{$succeeNUM}</font> 条,髮送失败的短信有 <font color=red>{$failNUM}</font> 条","index.php?lfj=$lfj&job=send",30);	
	}
}
elseif($action=="test"&&$Apower[mail_send])
{
	...
	else
	{
		$succeeNUM=intval($succeeNUM);
		$failNUM=intval($failNUM);
		jump("短信髮送完毕,髮送成功的短信有 <font color=red>{$succeeNUM}</font> 条,髮送失败的短信有 <font color=red>{$failNUM}</font> 条","index.php?lfj=$lfj&job=$action",30);	
	}
}
?>