...
function limitComment(){
	...
	if(limitTime>0){
		document.getElementById("comment_content").value='還賸'+limitTime+'秒,妳才可以再髮錶評價。';
...
function dblclick_label(){
	if(/jobs=show$/.test(location.href)){
		a=confirm('妳是否要退出標签琯理');
		if (a){
			window.location.href=location.href+'abc';
		}
	}else{
		b=confirm('妳是否要进入標签琯理');
		if (b){
			url = location.href
			if (/\?/.test(url)){
