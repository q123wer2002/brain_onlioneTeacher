...
function qibo_player(url,width,height,type,ifautostart){
	...
	{
		document.write("此媒体文件地阯:"+url+"有误,請检查之!!!");
	}
}
...
FLV播放器参数名称
参数说明 默认值 
vcastr_file 方法2传递影片flv文件地阯参数，多個使用|分开 空 
vcastr_title 影片標題参数，多個使用|分开，与方法2配合使用 空 
vcastr_xml 方法3 传递影片flv文件地阯参数，样板参考 http://www.ruochi.com/product/vcastr2/vcastr.xml  vcastr.xml 
...
IsShowBar 控制栏显示参数 ：0表示不显示；1表示一直显示；2表示鼠標悬停時显示；3表示开始不显示，鼠標悬停後显示 2 
BarColor 播放控制栏颜色，颜色都以0x开始16进制数字表示 0x000033 
BarTransparent 播放控制栏透明度 60 
GlowColor 按键图標颜色，颜色都以0x开始16进制数字表示 0x66ff00 
IconColor 鼠標悬停時光晕颜色，颜色都以0x开始16进制数字表示 0xFFFFFF 
TextColor 播放器文字颜色，颜色都以0x开始16进制数字表示 0xFFFFFF 
LogoText 可以添加自己網站名称等信息(英文) 空 
LogoUrl 可以從外部读取logo图片,注意自己调整logo大小,支持图片格式和swf格式 空 
EndSwf 影片播放结束後,從外部读取swf文件，可以添加相关影片信息，影片分享等信息，需自己制作 空 
BeginSwf 影片开始播放之前,從外部读取swf文件，可以添加广告，或者網站信息，需自己制作 空 
IsShowTime 是否显示時間 : 0表示不显示時間，1表示显示時間 1 
BufferTime 影片缓冲時間，單位（秒） 2 
*/