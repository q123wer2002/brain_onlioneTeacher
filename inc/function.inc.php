...
function upfile($upfile,$array){
	...
	else
	{
		...
		{
			showerr("妳所上传的文件格式为:$filetype,而妳所在用户组仅允许上传的文件格式为:$groupdb[upfileType]",1);
		}
		elseif( !in_array($filetype,explode(" ",$webdb[upfileType])) )
		{
			showerr("妳所上传的文件格式为:$filetype,而系统仅允许上传的文件格式为:$webdb[upfileType]",1);
		}

		if( $groupdb[upfileMaxSize] && ($groupdb[upfileMaxSize]*1024)<$size )
		{
			showerr("妳所上传的文件大小为:".($size/1024)."K,而妳所在用户组仅允许上传的文件大小为:{$groupdb[upfileMaxSize]}K",1);
		}
		if( !$groupdb[upfileMaxSize] && $webdb[upfileMaxSize] && ($webdb[upfileMaxSize]*1024)<$size )
		{
			showerr("妳所上传的文件大小为:".($size/1024)."K,而系统仅允许上传的文件大小为:{$webdb[upfileMaxSize]}K",1);
		}
	}
...
function tempdir($file,$type=''){
	...
	if($type=='pwbbs'){
		...
		return $file;
	}elseif($webdb[mirror]&&!file_exists(ROOT_PATH."$webdb[updir]/$file")){	//FTP镜像點
...
function getpage($table,$choose,$url,$rows=20,$total=''){
	...
	for($j=$s;$j<=$b;$j++){
		if($j==$page){
			$show.=" <a href='#' class=\"ck\">$j</a>";
		}else{
			$show.=" <a href=\"$url&page=$j\" title=\"第{$j}頁\">$j</a>";
		}
	}
	$showpage="<a href=\"$url&page=1\" title=\"首頁\">首頁</A> <a href=\"$url&page=$uppage\" title=\"上一頁\">上一頁</A>  {$show}  <a href=\"$url&page=$nextpage\" title=\"下一頁\">下一頁</A> <a href=\"$url&page=$totalpage\" title=\"尾頁\">尾頁</A>";
    if($totalpage>1){
		return $showpage;
	}
}
...
function showerr($showerrMsg,$type=''){
	...
	if($type==1){
		...
		extract($GLOBALS);
	require(ROOT_PATH."template/default/head.htm");
		require(ROOT_PATH."template/default/showerr.htm");
	require(ROOT_PATH."template/default/foot.htm");
		$content=ob_get_contents();
		ob_end_clean();
		ob_start();
		if($webdb[www_url]=='/.'){
...
function add_user($uid,$money,$about=''){
	...
	$webdb[moneylog_num] = 100;		//只保存每個用户的最近100条記录
...
function En_TruePath($content,$type=1,$ifgetplayer=0){
	...
	if($type==1)
	{
		...
		$content=str_replace("$webdb[www_url]","$webdb[www_url]",$content);
	}
	else
	{
		//使用了远程附件镜像,要做特别处理,不局限于使用FTP
		if($webdb[mirror]){
			...
			$content=str_replace("$webdb[www_url]","$webdb[www_url]/$webdb[updir]",$content);
			$content=str_replace("$webdb[www_url]","$webdb[www_url]/$webdb[updir]",$content);
		}		
		$content=str_replace("$webdb[www_url]","$webdb[www_url]",$content);
		$content=str_replace("$webdb[www_url]","$webdb[www_url]",$content);
...
function ipfrom($ip) {
	...
	if( !is_file(ROOT_PATH.'inc/ip.dat') ){
		return '<a title><A HREF="http://down.qibosoft.com/ip.rar" title="點擊下载後,解压放到整站/inc/目录即可">IP库不存在,請點擊下载一個!</A></a>';
	}
...
/**
自定义模型當中,获取这三個select,radio,checkbox表單中类似“
1|中国
2|美国
”真实值
**/
...
//获取UNIX時間,主要是特别处理变成整数.不加引號08与8会不一样的结果,加引號是正常的
...
function check_imgnum($yzimg){
	...
	$time=$timestamp-1800;	//半小時前的验证碼失效.

	if($webdb[YzImg_letter_differ]){	//区别字母大小寫
		$SQL=" BINARY ";
	}
...
function get_member_tpl_js($type){
	global $webdb;	
	if(!$webdb[style_member]||!is_file($file=ROOT_PATH."template/default/$type.htm")){
		$file = ROOT_PATH."template/default/$type.htm";
	}
	return $file;
}
...
function select_city($name,$fid='',$value=false){
	global $db,$pre;	
	$show.="<select name='$name'><option value=''>請选择</option>";
	if($value){
