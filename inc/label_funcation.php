...
/**
*为了获取模板的標签,定义数组值,如果是旧標签的话.在後來会被数值替换.新標签的话继续是数组,也会进一步被处理
**/
...
function add_div($value,$tag,$type='code',$div_w='',$div_h='',$div_bgcolor='',$if_hf=''){
	...
	if($value){
		...
		//$value外面套一個table,目的是为了处理兼容性问題,有時標签过多,有個别標签无法显示
...
function Get_Title($format){
	...
	while($rs=$db->fetch_array($query)){
		...
		//有辅助模板時,要特别处理,CK代表截取了一条数据
		if($format[tplpart_2code]){
			if($format[sql2]){			//如果有SQL语句的话,要过滤雷同的記录
				if($rs2[id]==$rs[id]){
					$ck++;
					continue;
				}				
			}elseif(!$ck){				//没SQL语句,并且辅助模板有变量時,要截取一条記录,$content一般不会單独出现
	...
	foreach($listdb AS $key1=>$array1){
		$show.="<tr>";
		//有辅助模板時,第一個外循环,要特别处理一下,即内循环加多一個
		...
		foreach($array1 AS $key2=>$rs){
			...
			//有辅助模板時,第一個外循环,第一個内循环時,特别处理辅助模板
			if($tpl_2code&&$key1==0&&$jjj==1){
				...
				//自定义模型當中,获取这三個select,radio,checkbox表單中类似“1|中国”真实值
				if($format[SYS]=='form'){
...
function CMS_special($format){
	...
	if($getbbs)
	{
		...
		//针对辅助模板
		if($format[tplpart_2code]){
			//针对如果記录没有足够指定的条数,则不必去掉一条記录,否则就要去掉最後一条
			if(count($listdb)>($Rows-1)){
				array_pop($listdb);
			}
			//如果文章系统里不存在,则获取论坛的
...
/**
*把$url,$listurl这两個通用網阯做处理,以便获取其真实的網阯
**/
function make_ture_path($format,$rs){
	...
	elseif($format[SYS]=='fenlei'){
		...
		//分类里边调用,就使用標准的URL处理,被其它模块调用的话,就用动态处理
	...
	//wn指万能模块,normal指5.0版中的图库.视频,下载,FLASH等频道的列表与内容页網阯
...
function label_set_rs($format,$rs){
	...
	//標題
	$rs[smalltitle] && $rs[title] = $rs[smalltitle];	//如果有简短標題的话,就使用简短標題
...
function CMS_sp_article($format,$rsdb,$Rows){
	...
	//排序处理,默认排序的话,如果不是读取图片的话,只读取第一個ID即可
	//如果是默认,然而有图片的话.处理起來就比较麻烦
	if(!$_SQL2PIC&&$format[order]=='default'){
		$string2id=preg_replace("/([0-9]+)(.*)/is","\\1",$rsdb[aids]);
		$_SQL2ORDER="";
	}else{
		$string2id=$rsdb[aids];
		//有图片,按专題设定排序時,要全部读出來.方便对图片进行排序处理
		if($_SQL2PIC&&$format[order]=='default'){
			$_SQL2ORDER="";
			$getpic++;
		}else{
	...
	if($SQL2){
		...
		if($rs2){
			$_format=$format;
			$_format[CMS]='artcile';$_format[ctype]='article';		//伪造,以便获取URL地阯
	...
	//如果按专題排序的话,要把所有内容都读出來,这里读500是估算最多也就是500条而已
	...
	while($rs = $db->fetch_array($query)){
		...
		//务必注意::下面伪造信息,是为了得到文章的$url,$listurl地阯
		$_format=$format;
		$_format[CMS]='artcile';$_format[ctype]='article';		//伪造,以便获取URL地阯
...
function get_sp_article($format,$rsdb,$Rows){
	...
	if($SQL2){
		...
		if($rs2){
			$_format=$format;
			$_format[SYS]='artcile';			//伪造,以便获取URL地阯
			$rs2=label_set_rs($_format,$rs2);
		}			
	}
...
function get_sp_pwbbs($format,$rsdb,$Rows2,$rs2){
	...
	if($format[tplpart_2code]&&!$rs2){
		...
		if($rs3){
			$_format=$format;
			$_format[SYS]='pwbbs';	//伪造,以便获取URL地阯
			$rs3=label_set_rs($_format,$rs3);
		}
	}

	//主模板如果要获取栏目名称的话.要读多一個表
	$_SQL1=$_SQL2='';
...
function get_sp_dzbbs($format,$rsdb,$Rows2,$rs2){
	...
	if($format[tplpart_2code]&&!$rs2){
		...
		if($rs3){
			$_format=$format;
			$_format[SYS]='dzbbs';	//伪造,以便获取URL地阯
			$rs3=label_set_rs($_format,$rs3);
		}
	}
...
function run_label_php($listdb){
	if($listdb[0][urlDB]){//这個是为了得到URL地阯,因为不同栏目可自定义不同的URL		
