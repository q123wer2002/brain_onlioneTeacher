...
if($jobs=='show')
{
	...
	foreach($label_hf AS $key=>$value)
	{
		...
		{
			$label[$key] || $label[$key]=add_div("新標签,无内容",$key,'NewTag','','','','99');
		}
	}	
}
else
	...
	while( $rs=$db->fetch_array($query) ){
		...
		if( $rs[typesystem] )
		{
			if( !is_array($label["$rs[tag]"]) ){
				$label_rubbish[$rs[lid]]=$rs[tag];	//多余的旧標签,做個提示	
				continue;	//页面没有的標签.就不要读数据库
			}
		...
		elseif( $rs[type]=='code' )
		{
			...
			//纠正一下不完整的javascript代碼,不必做权限判断,普通用户也能删除
			if(eregi("<SCRIPT",$value)&&!eregi("<\/SCRIPT",$value)){
				if($delerror){
					$db->query("UPDATE `{$pre}label` SET code='' WHERE lid='$rs[lid]'");
				}else{
					die("<A HREF='$WEBURL?&delerror=1'>此“{$rs[tag]}”標签有误,點擊删除之!</A><br>$value");
				}			
			}
			//真实地阯还原
...
if($jobs=='show')
{
	...
	if($label_rubbish){
		if($delete_label_rubbish){
			...
			//echo "<CENTER><br><br>提醒:::<A HREF='$WEBURL&delete_label_rubbish=1'>有 ".count($label_rubbish)." 個冗余的標签,妳是否要删除它,點擊即可删除:".implode(",",$label_rubbish)."</A><br><br><br><br></CENTER>";
		}		
	}
	$label || $label=array();
	foreach($label AS $key=>$value)
	{
		//如果是旧標签的话.$value已经是具体数值了,或者为空了,而不是数组
		if(is_array($value))
		{
			$label[$key]=add_div("新標签,内容暂无:$key",$key,'NewTag');
		}
	}

	//$all_label在函数label_array()label_array_hf()中统计所有有效的標签
