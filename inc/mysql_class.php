...
Class MYSQL_DB {
	...
	function connect($dbhost, $dbuser, $dbpw, $dbname, $pconnect = '' ,$ifUC='') {
		...
		if($pconnect) {
			...
			if(!@mysql_connect($dbhost, $dbuser, $dbpw)) {				
				$this->Err('MYSQL 连接数据库失败,請确定数据库用户名,密碼设置正确<br>');
				if($ifUC){
					$this->uc_err();
				}
				exit;
		...
		if(!@mysql_select_db($dbname)){			
			$this->Err("MYSQL 连接成功,但當前使用的数据库 {$dbname} 不存在<br>");
			if($ifUC){
				$this->uc_err();
			}
			exit;
	...
	function Err($msg='') {
		...
		if(strstr($sqlerror,"Can't open file: '")){
			...
			echo "系统已自动修复数据库,請再次刷新網页,如果修复不成功,請重启数据库再修复<br>";
	...
	function uc_err(){
		echo '<hr><A HREF="http://bbs.qibosoft.com/read-forum-tid-270806.htm" target="_blank">整合DZ/UC出错,請點擊查看错误原因:http://bbs.qibosoft.com/read-forum-tid-270806.htm</A>';
		exit;
	}
}
