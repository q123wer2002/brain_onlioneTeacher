...
if(!$webdb[alipay_id]){
	showerr('系统没有设置支付宝收款帐號,所以不能使用支付宝在線支付');
}
...
else
{
	...
	$para = array(
			...
			'partner'		 => $webdb['alipay_partner'],		//合作商户號
			'return_url'	 => $array['return_url'],		//同步返迴
			'payment_type'	 => 1,							//默认为1,不需要修改
			'quantity'		 => 1,							//商品数量,强制为1,总額在price处算好
			'subject'		 => $array['title'],			//商品名称，必填
			'body'			 => $array['content'],			//商品描述，必填
			'out_trade_no'	 => $array['numcode'],			//商品外部交易號，必填（保证唯一性）
			'price'		 => $array['money'],				//总額
			'seller_email'	 => $webdb['alipay_id'],		//卖家郵箱，必填
			'logistics_fee'		=> '0.00',        			//物流配送费用
			'logistics_payment'	=> 'BUYER_PAY',   			//物流费用付款方式：SELLER_PAY(卖家支付)、BUYER_PAY(买家支付)、BUYER_PAY_AFTER_RECEIVE(货到付款)
			'logistics_type'	=> 'EXPRESS'    			//物流配送方式：POST(平郵)、EMS(EMS)、EXPRESS(其他快递)
		);
	ksort($para);
	$and='';
