...
if(!$webdb[tenpay_id]){
	showerr('系统没有设置财付通收款帐號,所以不能使用财付通在線支付');
}elseif(!$webdb[tenpay_key]){
	showerr('系统没有设置财付通密钥,所以不能使用财付通在線支付');
}
...
class tenpay_config
{
	...
	//以下每一项都必须要配置，并准确
	var $spid 				="1210110601";			//卖家帐號
	var $sp_key				="f1d6a1974041659c8650476dda14bb58";																		//密钥
	var $domain				="http://www.anodize.cn"	;	//商户網站域名
	var $tenpay_dir			="/tenpay2";		//财付通安装目录
	var $site_name			="中国铝氧化網";		//商户網站名称
	var $attach				="tencent_magichu";		//支付附加数据，非中文標准字符
	var $imgtitle			="财付通支付"; 			//图片说明
	var $imgsrc				="/image/tenpay_buy.gif";		//图片地阯
	var $pay_url			="https://www.tenpay.com/cgi-bin/v1.0/pay_gate.cgi"; 	//财付通支付網关地阯
...
class tenpay_online_payment  
{
	...
	function tenpay_check_config ()//检查配置文件项目
	{
		 ...
		 if (empty($this->tenpay_config->spid))
			 ...
			 {
				$retcode = "09008";
				$retmsg = "缺少图片地阯，默认设置为/tenpay/image/tenpay_buy.gif";
			 ...
			 {
				$retcode = "09009";
				$retmsg = "缺少支付網关地阯，将被设置为https://www.tenpay.com/cgi-bin/v1.0/pay_gate.cgi";
	...
	function tenpay_interface_pay ($bank_type,$desc,$purchaser_id,$sp_billno,$total_fee,$attach,$ip)
	{
		...
		if ($config_retcode!="0")
			die("請检查配置文件tenpay_config.php中的各配置项是否正确配置");
		...
		if (empty($sp_billno))
			 ...
			 if ($desc=="")
			 {
				...
				$retcode = "06002";
				$retmsg  = "缺少商品名称desc，将被默认设置为".$this->tenpay_config->site_name."訂單：" . $sp_billno;;
				$desc = $this->tenpay_config->site_name."訂單：" . $sp_billno;
				}
		...
		{
			if ($beta_switch == "1") //判断测试开关，如果开启测试，支付金額为1分 
			{
				$total_fee = "0";
...
if($_GET[signMsg]||$_GET[sign]){
	...
	{
		//验证MD5签名失败
		//植入业务逻辑处理，請注意金額單位是分，财付通有可能多次通知商户支付成功，需要对财付通的重复通知做去重处理
		$retcode = "2";
		$retmsg = "验证MD5签名失败";
		die( "验证MD5签名失败 "); 
	}  
