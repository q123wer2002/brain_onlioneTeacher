...
if(!$webdb[yeepay_id]){
	showerr('系统没有设置易宝支付收款商户编號,所以不能在線支付');
}elseif(!$webdb[yeepay_key]){
	showerr('系统没有设置易宝支付密钥,所以不能在線支付');
}
...
#	送货地阯
# 为"1": 需要用户将送货地阯留在易宝支付系统;为"0": 不需要，默认为 "0".
$p9_SAF = "0";
...
if($_GET[hmac]){
	#	只有支付成功時易宝支付才会通知商户.
	##支付成功迴调有两次，都会通知到在線支付請求参数中的p8_Url上：浏览器重定向;服务器點对點通讯.

	#	解析返迴参数.
	//$return = getCallBackValue($r0_Cmd,$r1_Code,$r2_TrxId,$r3_Amt,$r4_Cur,$r5_Pid,$r6_Order,$r7_Uid,$r8_MP,$r9_BType,$hmac);

	#	判断返迴签名是否正确（True/False）
	...
	if($bRet){
		...
		#	需要比较返迴的金額与商家数据库中訂單的金額是否相等，只有相等的情况下才认为是交易成功.
		#	并且需要对返迴的处理进行事务控制，进行記录的排它性处理，防止对同一条交易重复髮货的情况髮生.      	  	
			
			if($r9_BType=="1"){
				//echo "交易成功";
				//echo  "<br />在線支付页面返迴";
			}elseif($r9_BType=="2"){
				#如果需要应答机制则必须迴寫流,以success开头,大小寫不敏感.
				echo "success";
				echo "<br />交易成功";
				echo  "<br />在線支付服务器返迴";      			 
			}elseif($r9_BType=="3"){ 
...
else
{
	...
	#	商家设置用户購買商品的支付信息.
	##易宝支付平台统一使用GBK/GB2312编碼方式,参数如用到中文，請注意转碼

	#	商户訂單號,选填.
	##若不为""，提交的訂單號必须在自身账户交易中唯一;为""時，易宝支付会自动生成随机的商户訂單號.
	...
	#	商户接收支付成功数据的地阯,支付成功後易宝支付会向该地阯髮送两次成功通知.
	$p8_Url                     = $array[return_url];
	//$p8_Url						= $_REQUEST['p8_Url'];	

	#	商户扩展信息
	##商户可以任意填寫1K 的字符串,支付成功時将原样返迴.												
	$pa_MP						= 'endpay';

	#	银行编碼
	##默认为""，到易宝支付網关.若不需显示易宝支付的页面，直接跳转到各银行、神州行支付、骏網一卡通等支付页面，该字段可依照附录:银行列表设置参数值.			
