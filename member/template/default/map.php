...
if($lfjdb[C][endtime]&&$lfjdb[groupid]!=8){
	...
	$lfjdb[C][endtime]='長期有效';
}


@extract($db->get_one("SELECT COUNT(*) AS pmNUM FROM `{$pre}pm` WHERE `touid`='$lfjuid' AND type='rebox' AND ifnew='1'"));

@extract($db->get_one("SELECT COUNT(*) AS course_uNum FROM `{$pre}course_u` WHERE `u_id`='$lfjuid'"));
@extract($db->get_one("SELECT COUNT(*) AS CommentNum FROM `{$pre}fenlei_comments` WHERE `uid`='$lfjuid'"));
